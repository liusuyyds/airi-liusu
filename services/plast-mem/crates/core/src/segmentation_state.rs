...
pub async fn commit_segmentation_job(
  ...
  if !finalized_spans.is_empty() {
    let start_seqs = finalized_spans
      .iter()
      .map(|span| span.start_seq)
      .collect::<Vec<_>>();
    let existing_start_seqs = episode_span::Entity::find()
      .filter(episode_span::Column::ConversationId.eq(claim.conversation_id))
      .filter(episode_span::Column::StartSeq.is_in(start_seqs))
      .all(&txn)
      .await?
      .into_iter()
      .map(|span| span.start_seq)
    ...
    let active_models: Vec<episode_span::ActiveModel> = finalized_spans
      .iter()
      .filter(|span| !existing_start_seqs.contains(&span.start_seq))
