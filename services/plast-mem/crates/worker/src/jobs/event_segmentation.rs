...
use plastmem_event_segmentation::{
  ReviewedSegment, RuleSegOutput, SegmentClassification, derive_bucket_ranges,
  primitive_review_llm_segmenter, temporal_boundary_review_llm_segmenter, temporal_rule_segmenter,
};
...
const SEGMENTATION_REVIEW_TIMEOUT_SECONDS: u64 = 60;
...
async fn process_claimed_segment_range(
  ...
  let final_segments = review_segments_with_llm_or_rules(claimed_messages, &rule_output).await?;
...
async fn review_segments_with_llm_or_rules(
  claimed_messages: &[plastmem_core::ConversationMessage],
  ...
  let review_timeout = Duration::from_secs(
    APP_ENV
      ...
      .min(SEGMENTATION_REVIEW_TIMEOUT_SECONDS),
  );
  let llm_review = timeout(review_timeout, async {
  ...
  match llm_review {
    Ok(Ok(final_segments)) => Ok(final_segments),
    Ok(Err(err)) => {
      tracing::warn!(
        error = %err,
        "Segmentation LLM review failed; falling back to temporal rule segments"
      );
      Ok(rule_segments(rule_output))
    }
    Err(_) => {
      tracing::warn!(
        timeout_seconds = review_timeout.as_secs(),
        "Segmentation LLM review timed out; falling back to temporal rule segments"
      );
      Ok(rule_segments(rule_output))
    }
  }
...
fn rule_segments(rule_output: &RuleSegOutput) -> Vec<ReviewedSegment> {
  derive_bucket_ranges(rule_output)
    .into_iter()
    .map(|bucket| ReviewedSegment {
